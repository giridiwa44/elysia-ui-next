<template>
 <div>
    ...
    >
      ...
      <span class="text-secondary-900/20 dark:text-white font-display">Quick Search ...</span>
      <kbd class="ml-2 text-sm">CTRL+K</kbd>
    </button>

    <!-- Modal -->
    <Teleport to="body">
      <div v-if="open" class="fixed inset-0 bg-secondary-800/50 flex items-start justify-center z-50 p-4">
        <div class="bg-white dark:bg-gray-700 w-full max-w-xl rounded-lg shadow-lg overflow-hidden">
          <!-- Input -->
          ...
          <!-- Hasil Pencarian -->
          <div v-if="query && Object.keys(groupedResults).length" class="max-h-72 overflow-y-auto" ref="resultsContainer">
              ...
              <ul>
                <li v-for="(item, idx) in group" :key="item.url">
                  <a :href="item.url"
                    ...
                    :class="{
                      'bg-gray-200 dark:bg-gray-600': flatResults[selectedIndex] === item,
          ...
          <div v-else-if="query" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">
...
const handleKeydown = (e) => {
    ...
    if (open.value && flatResults.value.length > 0) {
        ...
        if (e.key === "Enter" && selectedIndex.value >= 0) {
            e.preventDefault();
            window.location.href = flatResults.value[selectedIndex.value].url;
        }
    }
};
